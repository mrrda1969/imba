...
export const Agencies: React.FC = () => {
  // Raw input for allowed suburbs
  const [allowedSuburbsInput, setAllowedSuburbsInput] = useState("");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingAgency) {
        const updatedAgency = await apiService.updateAgency(
          editingAgency._id,
          submitData
        );
        setAgencies(
  ...
  const handleEdit = (agency: Agency) => {
    setAllowedSuburbsInput((agency.allowed_suburbs || []).join(", "));
        ...
        <div>
          ...
          {agency.whatsapp_number && (
            <p className="text-xs text-gray-500">
              WhatsApp: {agency.whatsapp_number}
            </p>
          )}
        </div>
      ),
            ...
            <div className="flex flex-wrap gap-1">
              ...
              {agency.allowed_suburbs.length > 3 && (
                <span className="text-xs text-gray-500">
                  +{agency.allowed_suburbs.length - 3} more
                </span>
              )}
            </div>
          ) : (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, primary_suburb: e.target.value })
              }
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              value={allowedSuburbsInput}
              onChange={(e) => setAllowedSuburbsInput(e.target.value)}
              onBlur={() => {
                setFormData({
                  ...formData,
                  allowed_suburbs: allowedSuburbsInput
                    .split(",")
                    .map((s) => s.trim())
                    .filter((s) => s),
                });
